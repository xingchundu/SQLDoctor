...
from app_exception import AgentError, ConfigurationError
...
5) "optimized_sql" 必须是完整可执行的 SQL；若证据不足或改写风险高，请原样返回原始 SQL。
...
class SqlAgent:
    ...
    async def run(
        ...
        *,
        ...
        llm_out = await self._run_llm(sql, explain, plan_report, rag_block=rag_block)
        return SqlAgentPipelineResult(
            explain=explain,
            plan_analysis=plan_report.to_json_dict(),
            issues=llm_out.issues,
            suggestions=llm_out.suggestions,
            optimized_sql=llm_out.optimized_sql,
            rag_chunks=rag_meta,
        )
    ...
    async def _run_llm(
        ...
        *,
        ...
        messages = self._build_messages(sql, explain, plan_report, rag_block=rag_block)
        try:
