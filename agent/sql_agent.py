...
4) 请结合执行计划中的访问路径、索引利用、行数/基数估计、以及排序或临时物化等线索说明问题；\
术语须与上方【引擎与术语】一致，勿默认用其他引擎的列名（例如 PostgreSQL 不要用 MySQL 的 key/Extra 作为主描述）。
...
def _sql_agent_engine_context(dialect: SqlDialect) -> str:
    """按所选数据库注入 LLM 应使用的 EXPLAIN 术语与 SQL 方言约束。"""
    if dialect == SqlDialect.MYSQL:
        return (
            "【引擎与术语】目标：MySQL / MariaDB。\n"
            "阅读 [EXPLAIN_JSON] 时请使用 MySQL EXPLAIN 习惯：type、possible_keys、key、rows、filtered、Extra（如 Using filesort）等。\n"
            "issues/suggestions 须符合该引擎的优化常识；optimized_sql 须为 MySQL 可执行语法。"
        )
    if dialect == SqlDialect.POSTGRES:
        return (
            "【引擎与术语】目标：PostgreSQL。\n"
            "阅读计划时请使用 PostgreSQL 习惯：Seq Scan、Index Scan、Index Only Scan、Bitmap Heap/Index Scan、"
            "Nested Loop、Hash Join、Merge Join、Sort 以及 cost、rows 等；\n"
            "即使 JSON 里出现与 MySQL 类似的字段名（来自统一视图），也请在文字中按 PG 语义解释，勿默认按 MySQL 优化套路作答。\n"
            "optimized_sql 须为 PostgreSQL 可执行语法。"
        )
    if dialect == SqlDialect.ORACLE:
        return (
            "【引擎与术语】目标：Oracle。\n"
            "阅读计划时请使用 Oracle 习惯：TABLE ACCESS FULL、INDEX RANGE SCAN、NESTED LOOPS、HASH JOIN 以及 cost、cardinality 等；\n"
            "optimized_sql 须为 Oracle 可执行语法；注意版本差异，慎用未经验证的 Hint。\n"
            "勿默认按 MySQL 的 type/key/Extra 表述概括 Oracle 计划。"
        )
    return (
        f"【引擎与术语】目标方言：{dialect.value}。\n"
        "请按该引擎的常见执行计划用语与 SQL 语法作答。"
    )
...
class SqlAgent:
    ...
    def _build_messages(
        ...
        *,
        ...
        system = f"{_SQL_AGENT_SYSTEM}\n\n{_sql_agent_engine_context(dialect)}"
        human = (
